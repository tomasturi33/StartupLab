      ...
      <v-sheet 
        ...
        <v-row class="pa-4">
          ...
          <v-col cols="12">
            ...
            <label class="text--secondary">Podés ir completado de forma parcial y guardando con el botón ACTUALIZAR al pie de este formulario </label>
          </v-col>
  ...
  export default {
    ...
    methods:{
      ...
      async getMyIdea(){
        await axios.get('convocation/'+this.convocation_form.convocation_form.id).then(response =>{
          if(response.data.length!==0){
            ...
            this.entrepreneurship_needs_2 = response.data.convocation.idea_need

            this.video = response.data.convocation.video

            this.curriculum_idea = response.data.convocation.curriculum_idea

            this.vertical_works_id = response.data.convocation.vertical_works_id
      ...
      async saveMyIdea(){
        try {
          await axios.post('convocation',{
            ...
            this.$router.push({ name: 'convocation'})
          })
        } catch (error) {
      ...
      async updateMyIdea(){
        this.loading=true
        try {
          await axios.put('convocation/'+this.id,{
              title: this.title,
              executive_summary: this.executive_summary,
              problem_detected: this.problem_detected,
              proposed_solution: this.proposed_solution,
