...
<body>
    ...
    @if (count($entrepreneurship->idea_need) != 0)
        <label>¿Qué estas buscando de la convocatoria?:</label><br>
        
        @foreach ($entrepreneurship->idea_need as $need)
    ...
    @if (count($entrepreneurship->curriculum_idea) != 0)
        <label>Curriculum de la Idea:</label><br><br>
        
        @foreach ($entrepreneurship->curriculum_idea as $cv)
            <b>- {{ $cv->type }}</b><br>
